...
{
    auto& windowTarget = reg.windowRenderTarget();
    Texture& storageImage = reg.createTexture2D(windowTarget.extent(), Texture::Format::RGBA16F, Texture::Usage::StorageAndSample);

    const Buffer* cameraUniforms = reg.getBuffer(CameraUniformNode::name(), "buffer");
    ...
    BindingSet& bindingSet = reg.createBindingSet({ { 0, ShaderStageRTRayGen, m_tlas },
                                                    ...
                                                    { 2, ShaderStageRTRayGen, cameraUniforms } });
